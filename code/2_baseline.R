...
result_path <- './output/2_baseline/result.csv'
result_select_path <- './output/2_baseline/result_selct.csv'
pred_path <- './output/2_baseline/pred/'
...
# 實驗 1 ==============================================
experiment_ls <- list(
  'decision trees(depth=2)' = function(train, label){
    model <- rpart(formula(paste(label, '~', '.')),
                      data=train, control=rpart.control(maxdepth=2),
                      method="class")
  },
  ...
  'decision trees(depth=30)' = function(train, label){
    model <- rpart(formula(paste(label, '~', '.')),
                   data=train, control=rpart.control(maxdepth=30),
                   method="class")
  }
)
